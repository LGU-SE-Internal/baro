from rcabench_platform.v2.logging import timeit, logger
...
def rcaeval_load_service_names(input_folder: Path) -> list[str]:
    metrics = (
        pl.scan_parquet(input_folder / "simple_metrics.parquet")
        .select(pl.col("service_name"))
        .unique()
    )
    traces = (
        pl.scan_parquet(input_folder / "traces.parquet")
        .select(pl.col("service_name"))
        .unique()
    )
...
def rcabench_load_service_names(input_folder: Path) -> list[str]:
    traces = (
        pl.scan_parquet(input_folder / "abnormal_traces.parquet")
        .select(pl.col("service_name"))
        .unique()
    )
    logs = (
        pl.scan_parquet(input_folder / "abnormal_logs.parquet")
        .select(pl.col("service_name"))
        .unique()
    )
